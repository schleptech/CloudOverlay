...
velo_points = np.asarray(o3d.io.read_point_cloud("OverlayPCs/1648667025.703640506.pcd").points)
...
image = cv2.imread('OverlayImgs/1648667025.702278483.jpg')
...
def print_projection_plt(points, color, image):
    ...
    hsv_image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    cv2.imwrite("prePoints.jpg", image)
    colValues = set()
    for i in range(points.shape[1]):
        colValues.add(int(color[i]))
        cv2.circle(hsv_image, (np.int32(points[0][i]),np.int32(points[1][i])),2, (int(color[i]),255,255),-1)
    print(colValues)
    cv2.imwrite("postPoints.jpg", hsv_image)
    return  cv2.cvtColor(hsv_image, cv2.COLOR_HSV2BGR)
...
cv2.imwrite(args.OutputName,image)
print('Done')